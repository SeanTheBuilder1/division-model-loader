...
Mesh processMesh(aiMesh *mesh, const aiScene *scene)
{
	...
	// 1. diffuse maps
	std::vector<Texture> diffuseMaps = loadMaterialTextures(material, aiTextureType_DIFFUSE, "texture_diffuse", textures_loaded);
	textures.insert(textures.end(), diffuseMaps.begin(), diffuseMaps.end());
	// 2. specular maps
	std::vector<Texture> specularMaps = loadMaterialTextures(material, aiTextureType_SPECULAR, "texture_specular", textures_loaded);
	textures.insert(textures.end(), specularMaps.begin(), specularMaps.end());
	// 3. normal maps
	std::vector<Texture> normalMaps = loadMaterialTextures(material, aiTextureType_HEIGHT, "texture_normal", textures_loaded);
	textures.insert(textures.end(), normalMaps.begin(), normalMaps.end());
	// 4. height maps
	std::vector<Texture> heightMaps = loadMaterialTextures(material, aiTextureType_AMBIENT, "texture_height", textures_loaded);
...
void saveModel(const std::string& destination, Model* src_model){
	...
	for(int i=0; i< size; i = i+3){
		file.write((char*)&src_model->meshes[i/3].vertices[0], sizeof(Vertex)*mesh_sizes[i]);
		file.write((char*)&src_model->meshes[i/3].indices[0], sizeof(unsigned int)*mesh_sizes[i+1]);
...
bool loadModel(const std::string& source, Model* result_model){
    ...
    const aiScene *scene = import.ReadFile(source, aiProcess_Triangulate | aiProcess_FlipUVs);	
